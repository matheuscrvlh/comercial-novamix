    ...
    return (
        ...
        >
            ...
            {!loading && podeBuscar && linhas.length === 0 && (
                <p className="py-6 text-center text-sm text-gray-dark dark:text-dark-text-muted lg:hidden">Nenhum produto encontrado.</p>
            )}
            ...
            {!loading && linhas.length > 0 && (
                <div className="flex flex-col gap-3 lg:hidden">
                    {linhas.map((row) => (
                        <MobileCard key={row.IDSUBPRODUTO} onClick={() => setProdutoSelecionado(row.IDSUBPRODUTO)}>
                            <div className="flex items-start justify-between gap-3">
                                ...
                                <Info className="h-4 w-4 shrink-0 text-gray-dark dark:text-dark-text-muted" />
                            </div>
                            <div className="mt-2 flex flex-col divide-y divide-gray-base/10 dark:divide-dark-border/60">
                                ...
                                <CardField label="Divisão" value={row.DESCRDIVISAO ?? '—'} />
                                <CardField label="Seção" value={row.DESCRSECAO ?? '—'} />
                                <CardField label="Grupo" value={row.DESCRGRUPO ?? '—'} />
                                <CardField label="Subgrupo" value={row.DESCRSUBGRUPO ?? '—'} />
                                <CardField
                                    label="Status"
                                    value={
                                        <span
                                            className={`rounded-full px-2 py-0.5 text-xs font-medium ${
                                                row.FLAGINATIVO === 'F' ? 'bg-green-base/10 text-green-base' : 'bg-red-base/10 text-red-base'
                                            }`}
                                        >
